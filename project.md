...
### ARQUITECTURA AUTH WITH COOKIES SESSION

Browser ──login──> Supabase Auth ──sessão em COOKIES (via @supabase/ssr)
│
▼
middleware.js (corre em cada request a /admin|/student|/teacher|/coordinator)
│ • refresca a sessão (cookies)
│ • sem sessão → redirect /auth/login
│ • role ≠ 1.º segmento do path → redirect para o dashboard do próprio role
▼
(dashboard)/layout.js (Server Component, async)
│ • getCurrentUser() → { user, role, name }
│ • passa role+user à Sidebar/Header
▼
